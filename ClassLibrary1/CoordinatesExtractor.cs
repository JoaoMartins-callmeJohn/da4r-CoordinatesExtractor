...
namespace CoordinatesExtractor
{
	...
	public class CoordinatesExtractor : IExternalDBApplication
	{
		...
		private void createIssue(InputParams inputParameters, Coordinates correctCoordinates, BasePoint sp, BasePoint pbp, string fileName)
		{
			...
			request.AddParameter("application/json", "{"+$"\n        \"title\": \"{fileName} Coordinates issue\",\n        \"description\": \"link: http://localhost:8080/issues/?urn={inputParameters.versionUrn}&output={inputParameters.outputUrn}; file: {fileName};project_base_point: {pbp.Position.ToString()};project_survey_point: {sp.Position.ToString()};correct_base_point: {correctCoordinates.basePoint.ToString()};correct_survey_point: {correctCoordinates.surveyPoint.ToString()};\",\n        \"issueSubtypeId\": \"{inputParameters.issueSubTypeId}\",\n        \"status\": \"open\",\n        \"assignedTo\": \"{inputParameters.userId}\",\n        \"assignedToType\": \"user\",\n        \"published\": true\n      " +"}", ParameterType.RequestBody);
